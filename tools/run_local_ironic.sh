...
# metal3-dev-env. It launches Ironic (and its supporting containers) locally
# using the container runtime, instead of deploying it inside the management
# cluster.
#
# It is intended for local/development use only and is executed by
# metal3-dev-env only when USE_IRSO=false and IRONIC_RUN_LOCAL=true. By default
# CI deploys Ironic in-cluster via the ironic-standalone-operator (IRSO) and
# does NOT run this script.

# TOOLSDIR is the directory containing this script (metal3-dev-env/tools). It is
# used to locate the companion remove_local_ironic.sh script.
...
# Bracket the provisioning IP for IPv6 so the URL is well-formed
# (e.g. http://[fd2e:...]/images instead of http://fd2e:.../images).
if [[ "${PROVISIONING_IP}" = *":"* ]]; then
    PROVISIONING_HOST="[${PROVISIONING_IP}]"
else
    PROVISIONING_HOST="${PROVISIONING_IP}"
fi
CACHEURL="${CACHEURL:-"http://${PROVISIONING_HOST}/images"}"
...
"${TOOLSDIR}/remove_local_ironic.sh"
