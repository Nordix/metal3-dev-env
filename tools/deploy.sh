...
#
# It is intended for local/development deployments only, selected by setting
# USE_IRSO=false in metal3-dev-env. By default CI deploys Ironic in-cluster via
# the ironic-standalone-operator (IRSO) and does NOT use this script.
#
# The script operates on the baremetal-operator checkout pointed to by BMOPATH.
# When BMOPATH is not set (e.g. when the script is run from within a
# baremetal-operator checkout) it falls back to the parent directory of this
# script, preserving the original behavior.
