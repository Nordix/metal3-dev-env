...
start_management_cluster()
{
    ...
    if [[ "${BOOTSTRAP_CLUSTER}" = "kind" ]]; then
        ...
        # Assign only the base provisioner IP here (not the cluster VIP). With
        # IRSO the keepalived container owns CLUSTER_BARE_METAL_PROVISIONER_IP,
        # so pinning it statically would conflict with keepalived's VRRP
        # management. This mirrors the IPv4 branch below.
        if [[ "${BARE_METAL_PROVISIONER_SUBNET_IPV6_ONLY:-}" = "true" ]]; then
