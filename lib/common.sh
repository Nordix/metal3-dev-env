...
export SUPPORTED_DISTROS=(centos9 rhel9 centos10 rhel10 ubuntu20 ubuntu22 ubuntu24 sles15)
...
case "${FSTYPE}" in
  ...
  ;;
  'xfs')
    # sles has xfs_info under /usr/sbin and it needs to run with sudo
    if [[ "${OS}" = "sles"* ]]; then
      # shellcheck disable=SC2143
      if [[ $(sudo xfs_info "${FILESYSTEM}" | grep -q "ftype=1") ]]; then
