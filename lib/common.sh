...
if [[ "${DISTRO}" = "ubuntu24" ]] || [[ "${DISTRO}" = "centos10" ]]; then
    export ANSIBLE_VERSION="${ANSIBLE_VERSION:-10.7.0}"
else
    export ANSIBLE_VERSION="${ANSIBLE_VERSION:-9.13.0}"
fi

# Test and verification related variables
SKIP_CONTROLLERS="${SKIP_CONTROLLERS:-false}"
SKIP_RETRIES="${SKIP_RETRIES:-false}"
SKIP_NODE_IMAGE_PREPULL="${SKIP_NODE_IMAGE_PREPULL:-false}"
