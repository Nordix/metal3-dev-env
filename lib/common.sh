...
export NUM_OF_CONTROLPLANE_REPLICAS="${NUM_OF_CONTROLPLANE_REPLICAS:-"1"}"
...
export KUBERNETES_VERSION=${KUBERNETES_VERSION:-"v1.24.1"}
...
if [ "${KUBERNETES_VERSION}" == "v1.21.2" ]; then
  ...
  export KIND_NODE_IMAGE_VERSION=${KIND_NODE_IMAGE_VERSION:-"v1.24.1"}
  # Minikube version (if EPHEMERAL_CLUSTER=minikube)
...
# Ansible version
export ANSIBLE_VERSION=${ANSIBLE_VERSION:-"5.6.0"}
...
if [ "${NUM_NODES}" -lt "$((NUM_OF_CONTROLPLANE_REPLICAS + NUM_OF_WORKER_REPLICAS))" ]; then
    ...
    echo "NUM_NODES: ${NUM_NODES} < (${NUM_OF_CONTROLPLANE_REPLICAS} + ${NUM_OF_WORKER_REPLICAS})"
    exit 1
fi
